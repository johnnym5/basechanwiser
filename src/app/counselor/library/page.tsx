      ...
      <div className="space-y-10 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-800 pb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 font-bold uppercase tracking-widest mt-1">Manage, update, and deploy learning materials to mission packs.</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {resources.length === 0 ? (
                ...
                <div key={res.id} className="bg-slate-900/50 p-8 rounded-[32px] border border-slate-800 shadow-sm space-y-5 group transition-all hover:border-indigo-500/50">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => openEdit(res)} className="p-2.5 rounded-full bg-blue-900/30 text-blue-400 border border-blue-900/50 transition-colors hover:bg-blue-600 hover:text-white"><Edit className="w-4 h-4" /></button>
                      <button onClick={() => handleDelete(res.id)} className="p-2.5 rounded-full bg-rose-900/30 text-rose-400 border border-rose-900/50 transition-colors hover:bg-rose-600 hover:text-white"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </div>
                  <div>
                  ...
                  <div className="pt-5 border-t border-slate-800 flex items-center justify-between">
                    <span className="text-[9px] font-black uppercase text-indigo-400 bg-indigo-500/10 px-3 py-1.5 rounded-lg tracking-tighter border border-indigo-500/20">
                        {packs.find(p => p.id === res.linkedPackId)?.title || "Unlinked"}
                    </span>
