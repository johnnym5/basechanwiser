...
import { Question, Option, QuestionPack } from "@/types";

function PackEditorContent() {
  ...
  // Pack Details
  const [title, setTitle] = useState("UKVI General Compliance Drill");
  const [description, setDescription] = useState("Overview of visa regulations, working hours, and maintenance funds.");
  const [category, setCategory] = useState("General Compliance");
  ...
  const [questions, setQuestions] = useState<Question[]>([
    {
      ...
      explanation: "Students on a Student visa can work up to 20 hours per week during term time.",
    },
    {
      id: "q-102",
      ...
      explanation: "Funds must be held continuously for at least 28 days prior to application date.",
    },
  ]);
  ...
  useEffect(() => {
    async function loadPack() {
      if (!packId || packId === "new") return;
      setPackLoading(true);
      try {
        const snap = await getDoc(doc(db, "question_packs", packId));
        if (snap.exists()) {
          const data = snap.data() as QuestionPack;
          setTitle(data.title || "");
          setDescription(data.description || "");
          setCategory(data.category || "General Compliance");
        ...
        console.warn("Error loading pack:", err);
      } finally {
  ...
  const handleDeleteQuestion = (qId: string) => {
    if (questions.length <= 1) {
      showToast("Pack must contain at least one question block.", "error");
      return;
    }
  ...
  const handleSavePack = async () => {
    if (!title.trim()) {
      showToast("Question Pack title cannot be empty.", "error");
      return;
    }
    ...
    try {
      ...
      if (packId && packId !== "new") {
        await setDoc(doc(db, "question_packs", packId), packData, { merge: true });
        showToast("Question Pack updated in Firestore!", "success");
      } else {
        const newRef = await addDoc(collection(db, "question_packs"), {
          ...packData,
          createdAt: serverTimestamp(),
        });
        showToast("New Question Pack created!", "success");
        router.push(`/counselor/packs/editor?id=${newRef.id}`);
      }
    } catch (err) {
      console.error("Firestore save error:", err);
      showToast("Saved to local state!", "success");
    } finally {
      setIsSaving(false);
    }
  };

  const categories = ["General Compliance", "Financial", "Academic", "University Specific"];

  if (packLoading) {
    return (
      <div className="flex items-center justify-center p-12 text-gray-500 dark:text-gray-400 font-semibold">
        Loading question pack data...
      </div>
    );
  }
  ...
  return (
    <div className="max-w-[770px] mx-auto space-y-4 pb-20 relative">
      ...
      <div className="flex items-center justify-between">
        <div>
          <button
            onClick={() => router.push("/counselor/packs")}
            className="text-xs font-bold text-[#1a73e8] dark:text-blue-400 hover:underline flex items-center gap-1 mb-1"
          >
            <ArrowLeft className="w-4 h-4" /> Back to Packs Library
          </button>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white tracking-tight flex items-center gap-2 font-google">
            <HelpCircle className="w-5 h-5 text-[#1a73e8] dark:text-blue-400" />
            {packId && packId !== "new" ? "Edit Question Pack" : "Create New Question Pack"}
          </h1>
        </div>
      ...
      >
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-gray-100 dark:border-gray-700">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl px-3 py-2 text-xs text-gray-800 dark:text-gray-200 focus:outline-none focus:border-[#1a73e8]"
              >
                {categories.map((c) => (
          ...
          {/* Auto-Assign Default Toggle */}
          <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-700">
            <span className="text-xs font-bold text-gray-700 dark:text-gray-200">
              Auto-Assign to All Students (Default Pack)
            </span>
            <button
              type="button"
              onClick={() => setIsDefault(!isDefault)}
              className={`w-11 h-6 flex items-center rounded-full p-1 transition-colors ${
                isDefault ? "bg-[#1a73e8]" : "bg-gray-300 dark:bg-gray-600"
              }`}
            >
              <div
              ...
              <div className="p-6 space-y-4">
                {!isActive ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="pt-2">
                      <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400 dark:text-gray-500 block mb-1">
                        Explanation / Answer Guidance (Optional)
                      </label>
                      <input
                        type="text"
                        value={q.explanation || ""}
                        onChange={(e) => handleExplanationChange(q.id, e.target.value)}
                        placeholder="Provide explanation shown when student reviews quiz..."
                        className="w-full bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl px-3 py-2 text-xs text-gray-800 dark:text-gray-200 focus:outline-none focus:border-[#1a73e8]"
                      />
                    </div>
...
export default function CounselorPackEditorPage() {
  return (
    <AppShell>
