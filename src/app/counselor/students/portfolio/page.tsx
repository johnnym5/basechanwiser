...
import { PackField, DEFAULT_PACK_FIELDS } from "@/types/pack";
...
const renderOverallStatus = (status: string) => {
  const base =
    "inline-flex items-center justify-center px-2.5 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap border transition-all duration-200";
  ...
  return (
    <span className={`${base} ${styles.bg} ${styles.text} ${styles.border}`}>
      <span
        className={`w-1.5 h-1.5 rounded-full mr-1.5 ${styles.dot} shadow-[0_0_8px_rgba(0,0,0,0.5)]`}
      ></span>
      {label}
    </span>
  );
...
function PortfolioContent() {
  ...
  // Dynamic Pack Fields State
  const [globalFields, setGlobalFields] = useState<PackField[]>(DEFAULT_PACK_FIELDS);
  const [packFields, setPackFields] = useState<PackField[]>(DEFAULT_PACK_FIELDS);

  // Default Tab State
  const [activeTab, setActiveTab] = useState<
    "dossier" | "mock" | "quiz" | "engagement"
  >("dossier");
  ...
  const canEdit =
    ...
    userProfile?.role === "Head of Compliance" ||
    student?.assignedCounselorId === user?.uid;
  ...
  useEffect(() => {
    ...
    async function fetchPortfolio() {
      ...
      try {
        ...
        // Fetch Student Details & Portfolio Answers
        const guideSnap = await withTimeout(
          getDoc(doc(db, "Users", id!, "portfolio", "study_guide")),
          10000
        );
        const packSnap = await withTimeout(
          getDoc(doc(db, "Interview_Packs", id!)),
          10000
        );

        if (isMounted) {
          let combinedPack: Record<string, any> = {};
          if (packSnap.exists()) combinedPack = { ...packSnap.data() };
          if (guideSnap.exists())
            combinedPack = { ...combinedPack, ...guideSnap.data() };
        ...
        // Fetch Global Default Pack Config
        let activeGlobal = DEFAULT_PACK_FIELDS;
        try {
          const globalConfigSnap = await withTimeout(
            getDoc(doc(db, "interview_pack_configs", "default")),
            5000
          );
          if (globalConfigSnap.exists() && globalConfigSnap.data().fields) {
            activeGlobal = globalConfigSnap.data().fields;
          }
        } catch (e) {
          console.warn("Global default pack config fetch failed, using fallback:", e);
        }

        if (isMounted) setGlobalFields(activeGlobal);

        // Fetch Student Pack Config Override
        let activeStudentFields = activeGlobal;
        try {
          const studentConfigSnap = await withTimeout(
            getDoc(doc(db, "Users", id!, "portfolio", "pack_config")),
            5000
          );
          if (studentConfigSnap.exists()) {
            const data = studentConfigSnap.data();
            if (data.fields && Array.isArray(data.fields) && data.fields.length > 0) {
              activeStudentFields = data.fields;
            } else if (data.customFields && Array.isArray(data.customFields)) {
              activeStudentFields = [...activeGlobal, ...data.customFields];
            }
          }
        } catch (e) {
          console.warn("Student pack config fetch failed:", e);
        }

        if (isMounted) setPackFields(activeStudentFields);
  ...
  // Pack Customization Handlers
  const handleAddQuestion = async (newField: PackField) => {
    if (!id) return;
    const updated = [...packFields, newField];
    setPackFields(updated);
    try {
      await setDoc(
        doc(db, "Users", id, "portfolio", "pack_config"),
        {
          fields: updated,
      ...
      setToast({ message: "Question added to student pack", type: "success" });
    } catch (err) {
      console.error("Error adding question:", err);
      setToast({ message: "Failed to save custom question", type: "error" });
    } finally {
  ...
  const handleDeleteQuestion = async (fieldId: string) => {
    if (!id) return;
    const updated = packFields.filter((f) => f.id !== fieldId);
    setPackFields(updated);
    try {
      await setDoc(
        doc(db, "Users", id, "portfolio", "pack_config"),
  ...
  const handleResetToDefault = async () => {
    if (!id) return;
    try {
      await deleteDoc(doc(db, "Users", id, "portfolio", "pack_config"));
      setPackFields(globalFields);
      setToast({ message: "Pack reset to global default", type: "success" });
    } catch (err) {
      console.error("Error resetting pack:", err);
      setToast({ message: "Failed to reset pack", type: "error" });
    } finally {
      setTimeout(() => setToast(null), 3000);
    }
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      setStudent((prev) =>
        prev ? { ...prev, readinessStatus: calculatedStatus } : null
      );
      setInterviewPack({ ...interviewPack, ...formData } as InterviewPack);
      setIsEditing(false);
      setToast({
        message: `Data Synced. Readiness: ${calculatedStatus.toUpperCase()}`,
        type: "success",
      });
    } catch (err) {
      console.error(err);
  ...
  if (!student)
    return (
      <div className="p-20 text-center">
        <p className="text-gray-500 font-bold">Scholar not found.</p>
      </div>
    );
  ...
  const avgScore =
    attempts.length > 0
      ? Math.round(
          attempts.reduce(
            (acc, a) => acc + (a.score || a.scorePercentage || 0),
            0
          ) / attempts.length
        )
      : 0;
  ...
  const categories = Array.from(
    new Set(packFields.map((f) => f.category?.trim() || "General"))
  );
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-32 relative">
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-1">
            <button
              ...
              className="text-[10px] font-black uppercase text-blue-500 flex items-center gap-1 hover:underline mb-1"
            >
              <ArrowLeft className="w-3 h-3" /> Back to Student Directory
            </button>
        ...
        <div className="flex items-center gap-4">
          ...
          {canEdit && (
            <button
              ...
              disabled={isSaving}
              className={`py-3 px-6 rounded-2xl font-black text-sm flex items-center gap-2 transition-all ${
                isEditing
                  ? "bg-emerald-600 text-white shadow-emerald-500/20"
                  : "bg-white dark:bg-[#1E293B] text-gray-700 dark:text-white border border-gray-200 dark:border-slate-700"
              }`}
            >
              {isSaving ? (
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            val: `${student.learningProgress || 0}%`,
            icon: TrendingUp,
            c: "text-purple-500",
          ...
          {
            label: "Pack Status",
            val: interviewPack?.status || "Not Started",
            icon: FileText,
            c: "text-amber-500",
          ...
          <div
            key={i}
            className="bg-white dark:bg-[#1E293B] p-6 rounded-[32px] border border-gray-100 dark:border-slate-800 shadow-sm space-y-2 group transition-all hover:shadow-md"
          >
            <div
              className={`w-10 h-10 rounded-2xl ${stat.bg} ${stat.c} flex items-center justify-center mb-2 group-hover:scale-110 transition-transform`}
            >
        ...
        <div className="p-8 md:p-12">
          {activeTab === "dossier" && (
            <div className="space-y-8">
              {isEditing ? (
                <StudentDossierEditor
                  fields={packFields}
                  formData={formData}
                  onChange={(updates) =>
                    setFormData((prev) => ({ ...prev, ...updates }))
                  }
                  onAddField={handleAddQuestion}
                  onDeleteField={handleDeleteQuestion}
                  onResetToDefault={handleResetToDefault}
                  canEdit={canEdit}
                />
              ) : (
                <div className="space-y-10 animate-in fade-in duration-300">
                  {!interviewPack && Object.keys(formData).length === 0 ? (
                    <EmptyState
                      icon={FileQuestion}
                    ...
                    <>
                      {/* Dynamic Grouped View Mode */}
                      {categories.map((cat) => {
                        const catFields = packFields.filter(
                          (f) => (f.category?.trim() || "General") === cat
                        );
                      ...
                      <div className="pt-4 border-t border-gray-100 dark:border-slate-800 flex items-center gap-3">
                        <div className="flex items-center gap-3 p-5 bg-blue-50/50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded-3xl w-full">
                          <div className="w-10 h-10 rounded-xl bg-blue-500 text-white flex items-center justify-center shadow-lg">
                            <CheckCircle2 className="w-5 h-5" />
                          </div>
                          <span className="text-[11px] font-black uppercase text-blue-700 dark:text-blue-400 tracking-widest">
          ...
          {activeTab === "mock" && id && (
            <div className="animate-in fade-in duration-500">
              <StudentProfileView studentId={id} hideHeader />
            </div>
          )}
              ...
              {attempts.length === 0 ? (
                ...
                <div className="space-y-4">
                  {attempts.map((a) => {
                    ...
                    const isPass =
                      (a.scorePercentage || a.score || 0) >= 80;
                    return (
                      <div
                        key={a.id}
                        className="bg-white dark:bg-[#1E293B] rounded-3xl border border-gray-100 dark:border-slate-800 overflow-hidden shadow-sm"
                      >
                        <div
                          onClick={() =>
                            setExpandedAttemptId(isExpanded ? null : a.id)
                          }
                          className="p-6 cursor-pointer flex items-center justify-between gap-4 hover:bg-gray-50/50 dark:hover:bg-[#0F172A]/50 transition-colors"
                        >
                          <div className="flex items-center gap-4">
                            <div
                            ...
                            <div>
                              <h4 className="text-sm font-black dark:text-white uppercase tracking-tighter">
                                {a.packTitle || "Learning Test"}
                              </h4>
                        ...
                        <AnimatePresence>
                          {isExpanded && (
                            ...
                            >
                              <h4 className="text-[10px] font-black uppercase text-gray-400 tracking-widest border-b border-gray-100 dark:border-slate-800 pb-3">
                                Question Breakdown
                              </h4>

                              <div className="space-y-4">
                                {(a.historyDetails || a.details || []).map(
                                  (q: any, qIdx: number) => (
                                    <div
                                      key={qIdx}
                                      className="bg-white dark:bg-[#1E293B] rounded-2xl p-5 border border-gray-100 dark:border-slate-800 shadow-sm space-y-3"
                                    >
          ...
          {activeTab === "engagement" && (
            <div className="space-y-8 animate-in fade-in duration-300">
              <h3 className="text-sm font-black uppercase tracking-widest text-gray-900 dark:text-white flex items-center gap-2">
                <HistoryIcon className="text-blue-500" size={18} /> Student
                  ...
                  {activityLogs.map((activity, idx) => {
                    ...
                    return (
                      <div key={activity.id || idx} className="relative pl-8">
                        {/* Timeline Node */}
                        <div
                          className={`absolute -left-[17px] top-1 w-8 h-8 rounded-full border-2 flex items-center justify-center z-10 ${visual.bg} ${visual.border}`}
                        >
                        ...
                        <div className="bg-slate-800/30 border border-slate-700/50 rounded-xl p-5 hover:bg-slate-800/50 transition-all group">
                          <div className="flex justify-between items-start mb-1">
                            <h4 className="font-bold text-slate-200 text-sm uppercase tracking-tight">
                          ...
                          <p className="text-xs text-slate-400 leading-relaxed">
                            {activity.description ||
                              activity.message ||
                              `Automated log for ${activity.type?.replace(
                                "_",
                                " "
                              )} event.`}
                          </p>
                          <p className="text-[10px] text-slate-600 font-black uppercase mt-3 tracking-widest group-hover:text-blue-500 transition-colors">
                            Authenticated:{" "}
                            {activity.studentName || student.displayName}
                          </p>
                        </div>
                      </div>
                    );
      ...
      {evalOpen && student && (
        <InterviewEvaluationModal
          ...
          counselorName={
            userProfile?.displayName || user?.displayName || "Counselor"
          }
        />
      )}
  ...
  return (
    ...
    >
      ...
      {active && (
        <motion.div
          layoutId="activeTab"
          className="absolute bottom-0 left-0 right-0 h-1 bg-blue-600 rounded-t-full shadow-[0_-4px_10px_rgba(37,99,235,0.5)]"
        />
      )}
    </button>
  );
...
function ViewField({ label, value }: { label: string; value: any }) {
  return (
    <div className="flex justify-between items-center text-xs border-b border-gray-50 dark:border-slate-800 pb-3">
      ...
      <span className="font-black dark:text-white text-right ml-4">
        {value || "N/A"}
      </span>
    </div>
  );
